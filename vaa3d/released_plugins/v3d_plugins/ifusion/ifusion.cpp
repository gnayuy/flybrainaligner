...
// reconstruct tiles into one stitched image
template <class Tdata>
bool ireconstructing(Tdata *pVImg, Y_VIM<REAL, V3DLONG, indexed_t<V3DLONG, REAL>, LUT<V3DLONG> > vim, Tdata intensityrange, bool b_rescaling, V3DPluginCallback2 &callback)
{
    //
    V3DLONG vx = vim.sz[0];
    V3DLONG vy = vim.sz[1];
...
bool ImageFusionPlugin::dofunc(const QString & func_name, const V3DPluginArgList & input, V3DPluginArgList & output, V3DPluginCallback2 & callback, QWidget * parent)
{
    ...
    else if (func_name == tr("iblender")) // linear blending
    {
        ...
        {
            //print Help info
            printf("\nUsage: v3d -x ifusion -f iblender -i <folder> -o <output_image> -p \"#s <save_blending_result zero(false)/nonzero(true)> #k <1/0(default)>\"\n");
            printf("k=false convert 12-bit images from [0, 4095] to 8-bit [0, 255]\n");
            printf("k=true (default) convert 12-bit images from [min, max] to 8-bit [0, 255]\n");
        ...
        bool b_saveimage = true; // #s save the blended image by default
        bool b_rescaling = true; // #k [min, max] to [0, 255] by default
                ...
                if(i+1 != argc) // check that we haven't finished parsing yet
                {
                    ...
                    if (*key == '#')
                    {
                        while(*++key)
                        {
                            ...
                            {
                                b_rescaling = (atoi( argv[i+1] ))?true:false;
                                i++;
                            }
                            else
                            {
        ...
        if(datatype_tile == V3D_UINT8)
        {
            ...
            //
            bool success = ireconstructing<unsigned char>((unsigned char *)pVImg, vim, 255, b_rescaling, callback);
            if(!success)
            {
        ...
        else if(datatype_tile == V3D_UINT16)
        {
            ...
            //
            bool success = ireconstructing<unsigned short>((unsigned short *)pVImg, vim, 4095, b_rescaling, callback);
            if(!success)
            {
                printf("Fail to call function ireconstructing! \n");
                return false;
