...
################################################################################
#
# Voxel Size: (63x) 0.19x0.19x0.19 (20x) 0.46x0.46x0.46
# Downsampling ratio: 0.4083x0.4083x0.25 (63x -> 20x)
# Upsampling ratio  : 2.4493x2.4493x4.00 (20x -> 63x) Then resize image.
# Dimensions : (63x) 2900x1450x872  (20x) 1184x592x218
# Dims("rec"): (63x) 3135x2508x1024 (20x) 1280x1024x256
#
################################################################################
...
TARSX=`readItemFromConf $CONFIGFILE "tgtFBSXDPX"`
TARTXEXT=`readItemFromConf $CONFIGFILE "tgtFBTXRECDPX"`
TARSXEXT=`readItemFromConf $CONFIGFILE "tgtFBSXRECDPX"`
...
RESSX_X=`readItemFromConf $CONFIGFILE "VSZX_63X_IS_DPX"`
RESSX_Y=`readItemFromConf $CONFIGFILE "VSZY_63X_IS_DPX"`
RESSX_Z=`readItemFromConf $CONFIGFILE "VSZZ_63X_IS_DPX"`
...
SUBSXREF=$((SUBSXREF-1));
MOVINGNIICR=${OUTPUT}"/subjectvoi63x_c"${SUBSXREF}".nii"
...
MOVINGDFRMDCR=${OUTPUT}"/subjectvoi63x_c"${SUBSXREF}"_deformed.nii"
...
SUBSXALINGEDTX=${FINALOUTPUT}"/Aligned20xScale.v3draw"
...
if ( is_file_exist "$SUBSXALINGEDTX" )
then
echo " SUBSXALINGEDTX: $SUBSXALINGEDTX exists"
else
#---exe---#
message " Resizing to 20x scale "
$Vaa3D -x ireg -f isampler -i $SUBSXALINGED -o $SUBSXALINGEDTX -p "#x $SRSTX #y $SRSTY #z $SRSTZ"
fi
...
$DIR/createVerificationMovie.sh -c $CONFIGFILE -k $TOOLDIR -w $WORKDIR -s $SUBSXDFRMD -i $TARSXCROP -r $((SUBSXREF+1)) -o ${FINALOUTPUT}/$ALIGNVERIFY
...
echo "alignment.space.name=Yoshi 20x Alignment Space" >> $META
...
echo "alignment.space.name=Yoshi 63x Alignment Space" >> $META
echo "alignment.resolution.voxels=${RESSX_X}x${RESSX_Y}x${RESSX_Z}" >> $META
echo "alignment.image.size=2900x1450x872" >> $META
