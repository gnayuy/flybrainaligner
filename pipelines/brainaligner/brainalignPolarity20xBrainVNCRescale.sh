...
echo "~ Executing flyalign20x_INT.sh"
$DIR/flyalign20x_INT.sh -c $CONFIG_FILE -t $TEMPLATE_DIR -k $TOOL_DIR -w $WORK_DIR -i "$INPUT1" -j "$INPUT2" -m \"$MOUNTING_PROTOCOL\" -e "$INPUT1_NEURONS" -f "$INPUT2_NEURONS"
