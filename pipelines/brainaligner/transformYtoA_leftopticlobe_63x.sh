...
#
# transform aligned left optic lobe from Yoshi template space to Arnim template space pipeline
...
### step 0 yflip signals and neurons
...
### step 1 mirror
SUBREFXF=${OUTPUT}"/subrefxflip.v3draw"
SUBSIGXF=${OUTPUT}"/subsigyxflip.v3draw"

NEURONXF=${OUTPUT}"/neuronyxflip.v3draw"

if ( is_file_exist "$SUBREFXF" )
then
echo "SUBREFXF: $SUBREFXF exists"
else
#---exe---#
message " step 1 mirror flip for SUBREF"
$Vaa3D -x ireg -f xflip -i $SUBREF -o $SUBREFXF
fi

if ( is_file_exist "$SUBSIGXF" )
then
echo "SUBSIGXF: $SUBSIGXF exists"
else
#---exe---#
message " step 1 mirror flip for SUBSIG"
$Vaa3D -x ireg -f xflip -i $SUBSIGYF -o $SUBSIGXF
fi

if ( is_file_exist "$NEURONXF" )
then
echo "NEURONXF: $NEURONXF exists"
else
#---exe---#
message " step 1 mirror for NEURONYF"
$Vaa3D -x ireg -f xflip -i $NEURONYF -o $NEURONXF
fi

### step 2 resize image
SUBREFRS=${OUTPUT}"/subrefxfliprs.v3draw"
SUBSIGRS=${OUTPUT}"/subsigxfliprs.v3draw"

NEURONRS=${OUTPUT}"/neuronyxfliprs.v3draw"
...
#---exe---#
message " step 2 resize SUBREFXF"
$Vaa3D -x ireg -f prepare20xData -o $SUBREFRS -p "#s $SUBREFXF #t $TARAS"
fi
...
#---exe---#
message " step 2 resize SUBSIGXF"
$Vaa3D -x ireg -f prepare20xData -o $SUBSIGRS -p "#s $SUBSIGXF #t $TARAS"
fi
...
#---exe---#
message " step 2 resize NEURONXF"
$Vaa3D -x ireg -f prepare20xData -o $NEURONRS -p "#s $NEURONXF #t $TARAS #k 1"
fi

### step 3 global align
SUBREFGA=${OUTPUT}"/subrefxfliprsga.v3draw"
SUBSIGGA=${OUTPUT}"/subsigxfliprsga.v3draw"

NEURONGA=${OUTPUT}"/neuronyxfliprsga.v3draw"
...
SUBREFDS=${OUTPUT}"/subrefxfliprsgads.v3draw"
SUBSIGDS=${OUTPUT}"/subsigxfliprsgads.v3draw"

NEURONDS=${OUTPUT}"/neuronyxfliprsgads.v3draw"
...
### step 5 local align
SUBREFLA=${OUTPUT}"/subrefxfliprsgadsla.v3draw"
SUBSIGLA=${OUTPUT}"/subsigxfliprsgadsla.v3draw"

NEURONLA=${OUTPUT}"/neuronyxfliprsgadsla.v3draw"
...
SUBREFRS=${OUTPUT}"/subrefxfliprsgadslars.v3draw"
SUBSIGRS=${OUTPUT}"/subsigxfliprsgadslars.v3draw"

NEURONRS=${OUTPUT}"/neuronyxfliprsgadslars.v3draw"
...
### step 7 rotate
SUBREFRT=${OUTPUT}"/subrefxfliprsgadslarsrt.v3draw"
SUBSIGRT=${OUTPUT}"/subsigxfliprsgadslarsrt.v3draw"

NEURONRT=${OUTPUT}"/neuronyxfliprsgadslarsrt.v3draw"
...
NEURONCP=${OUTPUT}"/neuronyxfliprsgadslarsrtcp.v3draw"
