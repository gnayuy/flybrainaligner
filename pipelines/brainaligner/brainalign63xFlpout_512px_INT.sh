#!/bin/bash
#
# fly brain alignment pipeline version 1.09, 2012/12/13
#

################################################################################
# 
# The pipeline is developed for solving 63x fly brain alignment problems.
# Target brain's resolution (0.38x0.38x0.38/0.19x0.19x0.38 um)
#
################################################################################
...
# templates
TARIS=`readItemFromConf $CONFIGFILE "tgtFBSXIS"`
TARISLANDMARKS=`readItemFromConf $CONFIGFILE "tgtFBSXISmarkers"`
TARAS=`readItemFromConf $CONFIGFILE "tgtFBSXAS"`
TARASLANDMARKS=`readItemFromConf $CONFIGFILE "tgtFBSXASmarkers"`
TARREF=`readItemFromConf $CONFIGFILE "REFNO"`
ATLAS=`readItemFromConf $CONFIGFILE "atlasFBTX"`
RES_X=`readItemFromConf $CONFIGFILE "VSZX_63X_IS"`
RES_Y=`readItemFromConf $CONFIGFILE "VSZY_63X_IS"`
RES_Z=`readItemFromConf $CONFIGFILE "VSZZ_63X_IS"`

TARIS=${TMPLDIR}"/"${TARIS}
TARISLANDMARKS=${TMPLDIR}"/"${TARISLANDMARKS}
TARAS=${TMPLDIR}"/"${TARAS}
TARASLANDMARKS=${TMPLDIR}"/"${TARASLANDMARKS}
ATLAS=${TMPLDIR}"/"${ATLAS}
...
echo "TARAS: $TARAS"
echo "TARREF: $TARREF"
echo "TARISLANDMARKS: $TARISLANDMARKS"
echo "TARASLANDMARKS: $TARASLANDMARKS"
echo "ATLAS: $ATLAS"
echo "RES_X: $RES_X"
echo "RES_Y: $RES_Y"
echo "RES_Z: $RES_Z"
...
$Vaa3D -x ireg -f isampler -i $SUBDeformed -o $LARS -p "#x $SMPLRATIO #y $SMPLRATIO #z $SMPLRATIO"
...
$Vaa3D -x ireg -f isampler -i $NEUBRAINALIGNEDYFLIP -o $NEUDeformed -p "#x $SMPLRATIO #y $SMPLRATIO #z $SMPLRATIO #i 1"
$Vaa3D -x ireg -f prepare20xData -o $NEUDeformedTX -p "#s $NEUDeformed #t $ATLAS #k 1"
fi
...
echo "alignment.image.size=1672x1024x360" >> $META
