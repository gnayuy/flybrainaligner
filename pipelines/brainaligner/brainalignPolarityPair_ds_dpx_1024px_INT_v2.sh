#!/bin/bash
#
# The alignment pipeline for high-resolution partial fly brain, version 2.0, April 10, 2014
...
$ANTS 3 -m  CC[ $FIXEDDS, $MOVINGDS, 0.75, 8] -m MI[ $FIXEDDS, $MOVINGDS, 0.25, 32] -t SyN[0.25]  -r Gauss[3,0] -o $SIMMETRIC -i $MAXITERSCC --initial-affine $INITAFFINE
fi
...
$Vaa3D -x ireg -f yflip -i $NEURONALIGNEDYFLIP -o $SXNEURONALIGNED
fi
...
#---exe---#
message " Padding zeros "
$Vaa3D -x ireg -f zeropadding -i $SXNEURONALIGNED -o $NEUSXWBS -p "#c $TARSXCROPCONFIG"
fi
...
$Vaa3D -x ireg -f prepare20xData -o $NEUSXALINGED -p "#s $NEUSXWBS #t $TARSX #k 1"
fi
...
then
# male fly brain
echo "alignment.space.name=Barry 63x Subsampled Alignment Space" >> $META
else
# female fly brain
